...
def init_pkg(pkg, repo_dest):
    ...
    with dir_path(repo_dest):
        ...
        diff --git a/project/examples/urls.py b/project/examples/urls.py
        index 8aa3c69..94fa3a4 100644
        --- a/project/examples/urls.py
        +++ b/project/examples/urls.py
        @@ -1,7 +1,7 @@
         from django.conf.urls.defaults import *


        -urlpatterns = patterns('project.examples.views',
        +urlpatterns = patterns('%(pkg)s.examples.views',
        ...
        index f90a506..33d3b59 100644
        --- a/project/settings/base.py
        +++ b/project/settings/base.py
        @@ -23,13 +23,13 @@ MINIFY_BUNDLES = {
             }
         }
...
def create_virtualenv(pkg, repo_dest):
    workon_home = os.environ.get('WORKON_HOME')
    venv_cmd = find_executable('virtualenv')
    python26 = find_executable('python2.6')
    if not python26:
        raise EnvironmentError('python2.6 is not installed or not '
...
def patch(hunk):
    ...
    rs = ps.wait()
    if rs != 0:
        raise subprocess.CalledProcessError('patch %s returned non-zeo exit '
                                            'status %s' % (file, rs))
...
def main():
    ...
    clone_repo(options.pkg, options.dest, options.repo, options.repo_dest,
               options.branch)
    venv = create_virtualenv(options.pkg, options.repo_dest)
